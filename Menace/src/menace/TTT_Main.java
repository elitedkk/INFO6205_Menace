...
public class TTT_Main {
	...
	Tree auto,firstPlay,secondPlay;
	Tree auto2;
	//private boolean[] remaining;
	char[][] field;
	public static Scanner sc;
	public boolean useProb;
	private Players p, human,human_sim1,human_sim2,menacePlayer1,menacePlayer2;
	Menace menace;
	...
	public TTT_Main() {
		...
		firstPlay = new Tree(new char[3][3], false, this);
		secondPlay = new Tree(new char[3][3], true, this);
		...
		menacePlayer2  = new Players("Menace", o, true, true);
		
		menacePlayer1  = new Players("Menace", o, true, true);
		human_sim2 = new Players("My Player",x, true, false);
		
		menace = new Menace(this);
		useProb = true;
		//field=new char[3][3];

		/* ---- in a new method called menaceTraining()------
		...
		/*
		logger.debug(getTimestamp()+" Number of wins = " + Menace.totalwin);
		logger.debug(getTimestamp()+" Number of losses = " + Menace.totallose);
		logger.debug(getTimestamp()+" Number of draw = " + Menace.totaldraw);
		...
		while(!mark) {
			if (total> 0) {
				...
				if(mark) {
					...
					if (isFinal) logger.info(getTimestamp()+" "+p.getName() + " marked x=" + Integer.toString(co[0]) + " and y=" + Integer.toString(co[1]));
				}
			}
			else break;
