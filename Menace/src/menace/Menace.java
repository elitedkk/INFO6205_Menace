...
public class Menace {
	...
	public void updateMapAfterCondition(String situation, int index, int points) {
		if(memory.containsKey(situation)) {
			...
			logger.debug(getTimestamp()+" Updating beads at " + situation + " at index= " + Integer.toString(index) + " by " + Integer.toString(points));
		}
		else {
			BeadsDistribution bd = new BeadsDistribution(index,points);
			memory.put(situation, bd);
			logger.debug(getTimestamp()+" Adding starting beads at " + situation + " at index= " + Integer.toString(index) + " by " + Integer.toString(points));
		}
	}
	...
	public int [] getAMove(char[][] field) {
		...
		logger.debug(getTimestamp()+" Getting move for " + situation);
		logger.debug(getTimestamp()+" Current number of elements in map= " + Integer.toString(memory.size()));
		...
		ys = co[1];
		sits = situation;
		logger.debug(getTimestamp()+" Try taking a move at " + Integer.toString(xs) + ", " + Integer.toString(ys));
	...
	private int[] checkMapForMove(String situation) {
		...
		/*if (TTT_Main.isFinal) {
			...
			logger.info(getTimestamp()+" The distribution of beads: " + str);
		}*/
		int getSum = bd.sum;
		if (loopThreshold<0) {
			logger.debug(getTimestamp()+" Ran out of beads for situation= " + situation);
